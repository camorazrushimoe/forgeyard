...
pub fn action_name(action: &Action) -> &'static str {
    match action {
        ...
        Action::BlockedOnError => "BlockedOnError",
    }
}
...
pub fn reason_for(action: &Action, io: &dyn WatchIo, plan: &Plan) -> &'static str {
    match action {
        ...
        Action::BlockedOnError => "review_fail_cap",
    }
}
...
pub fn encode_why(w: &Why) -> String {
    format!(
        "{{\n  \"schema\": {},\n  \"ts\": \"{}\",\n  \"project\": \"{}\",\n  \"action\": \"{}\",\n  \"reason\": \"{}\",\n  \"busy\": {},\n  \"spec_present\": {},\n  \"spec_stale\": {},\n  \"spec_sha\": \"{}\",\n  \"spec_verdict\": \"{}\",\n  \"plan_item\": \"{}\",\n  \"plan_status\": \"{}\",\n  \"cycles\": {},\n  \"crashes\": {},\n  \"pr_open\": {},\n  \"last_outcome\": \"{}\",\n  \"run_id\": \"{}\",\n  \"watch_pid\": {}\n}}\n",
        w.schema, esc(&w.ts), esc(&w.project), esc(&w.action), esc(&w.reason),
        if w.busy { "true" } else { "false" },
        if w.spec_present { "true" } else { "false" },
        if w.spec_stale { "true" } else { "false" },
        esc(&w.spec_sha), esc(&w.spec_verdict), esc(&w.plan_item), esc(&w.plan_status),
        w.cycles, w.crashes,
        if w.pr_open { "true" } else { "false" },
        esc(&w.last_outcome), esc(&w.run_id), w.watch_pid
    )
}
...
pub fn record_tick(root: &Path, project: &str, plan: &Plan, action: &Action, io: &dyn WatchIo) -> Result<bool> {
    ...
    let item = plan.in_flight().and_then(|i| plan.items.get(i)).or_else(|| plan.items.last());
    ...
    let sha = load_spec_source(root, project).map(|s| s.content_sha256).unwrap_or_default();
    let run_id = load_state(root, project).ok().map(|s| s.run_id).unwrap_or_default();
    let snap = Why {
        schema: 1, ts: now_rfc3339(), project: project.to_string(), action: name.clone(), reason: reason.clone(),
        busy: io.busy(), spec_present: io.spec_present(), spec_stale: io.spec_stale(), spec_sha: sha,
        spec_verdict: verdict.into(), plan_item: item.map(|i| i.id.clone()).unwrap_or_default(),
        plan_status: item.map(|i| i.status.as_str().to_string()).unwrap_or_default(),
        cycles: item.map(|i| i.cycles).unwrap_or(0), crashes: item.map(|i| i.crashes).unwrap_or(0),
        pr_open: if branch.is_empty() { false } else { io.pr_open(branch) },
        last_outcome: format_last_outcome(root, project), run_id, watch_pid: std::process::id(),
    };
    save_why(root, &snap)?;
    if changed {
        let mut ev = Event::new(project, Hook::Run);
        ev.agent = Agent::Watch; ev.step = "tick".into(); ev.status = name; ev.summary = reason.clone();
...
pub fn render_why(root: &Path, project: &str) -> Result<String> {
    ...
    if !dir.join("PROJECT.toml").exists() {
        return Err(crate::error::ForgeError::Precondition(format!("unknown project: {project}")));
    }
    ...
    let plan = crate::watch::load_plan(root, project).unwrap_or(Plan { default_branch: "main".into(), items: vec![] });
    let plan_line = match plan.in_flight().and_then(|i| plan.items.get(i)).or_else(|| {
        plan.items.iter().find(|i| !matches!(i.status, crate::watch::ItemStatus::Done | crate::watch::ItemStatus::Merged))
    }) {
        Some(it) => format!("item={} status={} cycles={} crashes={}", it.id, it.status.as_str(), it.cycles, it.crashes),
        None => "-".into(),
    };
    let (action, reason, updated, pr, outcome) = match &why {
        Some(w) => (dash(&w.action), dash(&w.reason), dash(&w.ts), if w.pr_open { "yes" } else { "no" }.to_string(), dash(&w.last_outcome)),
    ...
    let error = if !state.run_id.is_empty() {
        ...
        if p.exists() { format!("runs/{}/outcome.error", state.run_id) } else { "-".into() }
    } else { "-".into() };
...
fn spec_line(root: &Path, project: &str) -> String {
    ...
    let sha = load_spec_source(root, project).map(|s| s.content_sha256).filter(|s| !s.is_empty()).unwrap_or_else(|| "-".into());
    format!("{} {} sha={sha}", if present { "present" } else { "missing" }, if stale { "stale" } else { "fresh" })
}

fn last_stop_line(dir: &Path) -> String {
    let Ok(text) = fs::read_to_string(dir.join("events.jsonl")) else { return "-".into() };
    for line in text.lines().rev() {
        if line.trim().is_empty() { continue; }
        if let Ok(ev) = decode_event_line(line) {
            if ev.hook == Hook::Stop {
                let mut s = ev.status;
                if !ev.summary.is_empty() { s.push(' '); s.push_str(&ev.summary); }
...
fn row(key: &str, val: &str) -> String { format!("{:<12}{}\n", format!("{key}:"), val) }
...
fn json_bool(text: &str, key: &str) -> bool {
    ...
    if let Some(i) = text.find(&pat) {
        ...
        if let Some(rest) = rest.strip_prefix(':') { return rest.trim_start().starts_with("true"); }
    }
    false
}
...
fn now_rfc3339() -> String {
    ...
    let mut s = secs;
    let rem = s % 86400; s /= 86400;
    let hour = rem / 3600; let min = (rem % 3600) / 60; let sec = rem % 60;
    let mut year: i32 = 1970;
    loop {
        let ydays: u64 = if year % 4 == 0 && (year % 100 != 0 || year % 400 == 0) { 366 } else { 365 };
        if s >= ydays { s -= ydays; year += 1; } else { break; }
    }
    let leap = year % 4 == 0 && (year % 100 != 0 || year % 400 == 0);
    let mdays = [31u64, if leap { 29 } else { 28 }, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    let mut month = 1u32;
    for d in mdays { if s >= d { s -= d; month += 1; } else { break; } }
    let day = s + 1;
